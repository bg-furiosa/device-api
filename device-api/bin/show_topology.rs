...
use furiosa_device::{list_devices, Device, DeviceError, hwloc};
use itertools::join;
...
async fn main() -> Result<(), DeviceError> {
    ...
    for device1 in devices.iter() {
        ...
        for device2 in devices.iter() {
            let link_type = topology.get_link_type(
                &device1.busname().unwrap_or_default(),
                &device2.busname().unwrap_or_default(),
            );
